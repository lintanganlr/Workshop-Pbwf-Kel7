...
{{--
    ...
    <div class="nurse-list">
        @foreach ($tenagamedis as $nurse)
            <div class="nurse-card">
                <h3>{{ $nurse->nama_medis }}</h3>
                <p><strong>Spesialisasi:</strong> {{ $nurse->spesialisasi_medis }}</p>
                @if($nurse->image)
                    <img src="{{ asset('medisimg/'. $nurse->image)}}" alt="{{$nurse->image}}" width="100">
                @else
                    <p>Tidak ada foto</p>
                @endif
                <!-- Tambahkan tombol untuk detail perawat -->
            </div>
        @endforeach
    </div>
